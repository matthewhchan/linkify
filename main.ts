...
const linkRules = [
  {
    ...
    href: (match: string[]) => { return 'http://' + match[0]; },
  },
  {
    regex: /(?<!\S)([a-z]+)@(?!\S)/,
    href: (match: string[]) => { return 'mailto:' + match[1]; },
  }
];

function matchLinkRules(text: string): { text: string, href: string, index: number } | null {
...
function linkify(text: string): (string | Node)[] {
  ...
  let before = text.substring(0, link.index);
  let after = text.substring(link.index + link.text.length);
